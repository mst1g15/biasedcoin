...
#' Calculate L-optimal criterion assuming a linear model (with or without weights)
...
#' Calculate DA-optimal criterion assuming a linear model (with or without weights)
#' @param D design matrix
#' @param cr  matrix of contrasts
#' @param prior.scale prior scale parameter
...
#'
#' @export
coord.cr <- function(covar, k, cr, cr.lossfunc, prior.scale=100){
  ...
  for (m in 1:k){
    ...
    repeat{
      ...
      for (i in 1:n){
        ...
        plusD <- cr.lossfunc(D, cr, prior.scale, wr=NULL)

        D[i, "tmt"] <- 0 #calculate criterion for unit i assigned to tmt -1
        D[i, (j+3):(2*j+2)] <- rep(0, j)

        minusD <- cr.lossfunc(D, cr, prior.scale, wr=NULL)
...
#'  probabilities for treatment assignment, all values of optimalities (weighted L, DA, weighted DA), proportion of treatment=1,
#'  proportion of covariate in each group
#'  Type 1 error, true value of power, empirical value of power.
#
#' @export
...
cr.bayes.des <- function(covar, true.beta, sigma, threshold,  kappa, init, cr.lossfunc, k, wt=T, int=T, a=2, d=2, M=NULL, prior.scale=100,
  ...
  results <- list(D=D, Dms=Dms, y=y, all.M=all.M, all.V=all.V, all.a=all.a, all.d=all.d,
                  ...
                  all.alpha=all.alpha,     all.lopt=all.lopt, all.dawopt=all.dawopt, all.daopt = all.daopt,
                  all.power.emp = all.power.emp, all.power.th = all.power.th)
